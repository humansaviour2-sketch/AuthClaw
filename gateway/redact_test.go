...
func TestRedactEngine(t *testing.T) {
	...
	t.Run("Redaction_Masking_Strategy", func(t *testing.T) {
		...
		redacted, tokenMap, err := RedactPrompts(ctx, tenantID, []string{prompt})
		if err != nil {
	...
	t.Run("Latency_Overhead_Under_50ms", func(t *testing.T) {
		...
		for i := 0; i < 3; i++ {
			_, _, _ = RedactPrompts(ctx, tenantID, []string{prompt})
		}

		duration := ProfileRedaction("RedactPrompts", func() {
			_, _, err := RedactPrompts(ctx, tenantID, []string{prompt})
			if err != nil {
